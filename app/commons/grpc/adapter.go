...
type (
	// CadenceAdapter class for workflow sample helper.
...
func (h *CadenceAdapter) SetupServiceConfig(config *config.CadenceConfig) {
	if h.Service != nil {
		return
	}
	...
	h.Config.DomainName = config.Domain
	h.Config.HostNameAndPort = config.HostPort
	h.Config.ServiceName = config.Service
	...
	_, err = domainClient.Describe(context.Background(), config.Domain)
	if err != nil {
		logger.Info("Domain doesn't exist", zap.String("Domain", config.Domain), zap.Error(err))
	} else {
		logger.Info("Domain successfully registered.", zap.String("Domain", config.Domain))
	}
...
func (h *CadenceAdapter) StartWorkflow(
...
func (h *CadenceAdapter) StartWorkflowWithCtx(
	ctx context.Context,
	options client.StartWorkflowOptions,
	workflow interface{},
	args ...interface{},
...
func (h *CadenceAdapter) SignalWithStartWorkflowWithCtx(ctx context.Context, workflowID string, signalName string, signalArg interface{},
...
func (h *CadenceAdapter) RegisterWorkflow(workflow interface{}) {
	h.RegisterWorkflowWithAlias(workflow, "")
}

func (h *CadenceAdapter) RegisterWorkflowWithAlias(workflow interface{}, alias string) {
...
func (h *CadenceAdapter) RegisterActivity(activity interface{}) {
	h.RegisterActivityWithAlias(activity, "")
}

func (h *CadenceAdapter) RegisterActivityWithAlias(activity interface{}, alias string) {
	registryOption := registryOption{
		registry: activity,
		alias:    alias,
	}
...
func (h *CadenceAdapter) StartWorkers(domainName string, groupName string, options worker.Options) {
...
func (h *CadenceAdapter) QueryWorkflow(workflowID, runID, queryType string, args ...interface{}) {
	workflowClient, err := h.Builder.BuildCadenceClient()
	if err != nil {
		h.Logger.Error("Failed to build cadence client.", zap.Error(err))
		panic(err)
...
func (h *CadenceAdapter) ConsistentQueryWorkflow(
	valuePtr interface{},
...
func (h *CadenceAdapter) SignalWorkflow(workflowID, signal string, data interface{}) {
	workflowClient, err := h.Builder.BuildCadenceClient()
	if err != nil {
		h.Logger.Error("Failed to build cadence client.", zap.Error(err))
		panic(err)
...
func (h *CadenceAdapter) CancelWorkflow(workflowID string) {
	workflowClient, err := h.Builder.BuildCadenceClient()
	if err != nil {
		h.Logger.Error("Failed to build cadence client.", zap.Error(err))
		panic(err)
...
func (h *CadenceAdapter) registerWorkflowAndActivity(worker worker.Worker) {
